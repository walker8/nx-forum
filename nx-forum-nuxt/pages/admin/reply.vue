      ...
      <div class="table-wrapper">
        <el-table :data="tableData" v-loading="loading" border ref="tableRef">
          ...
          <el-table-column fixed="right" label="操作" width="180" align="center">
            <template #default="scope">
              <el-button-group v-if="deleted">
                <el-button type="primary" text bg size="small" @click="restoreBatch([scope.row.replyId])"
                  v-if="hasPermission('admin:comment:restore', forumId)">
                  还原
                </el-button>
              </el-button-group>
              <el-button-group v-else-if="auditStatus === AuditStatus.AUDITING">
                <el-button type="success" text bg size="small" @click="passBatch([scope.row.replyId])"
                  v-if="hasPermission('admin:comment:pass', forumId)">
                  通过
                </el-button>
                <el-button type="danger" text bg size="small" @click="rejectBatch([scope.row.replyId])"
                  v-if="hasPermission('admin:comment:reject', forumId)">
                  拒绝
                </el-button>
              </el-button-group>
              <el-button-group v-else>
                <el-button type="danger" text bg size="small" @click="deleteBatch([scope.row.replyId])"
                  v-if="hasPermission('admin:comment:delete', forumId)">
                  删除
                </el-button>
              </el-button-group>
